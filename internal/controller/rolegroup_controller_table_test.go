...
func TestRoleGroupReconciler_Reconcile_TableDriven(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(mc *MockControllerClient, ms *MockStatusWriter) {
				mc.On("Get", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
					Return(nil, apierrors.NewNotFound(schema.GroupResource{}, "rolegroup"))
			},
			expectedResult: ctrl.Result{},
			expectedError:  false,
		},
		...
		{
			name: "PostgreSQL connected, no vault - syncRoleGroup called",
			request: ctrl.Request{
				NamespacedName: types.NamespacedName{Name: "rg1", Namespace: "default"},
			},
			setupMocks: func(mc *MockControllerClient, ms *MockStatusWriter) {
...
func TestRoleGroupReconciler_SyncRoleGroup_TableDriven(t *testing.T) {
	tests := []struct {
		name            string
		...
		{
			name: "Sync role group - pg operation fails (no real DB), status update succeeds",
			roleGroup: &instancev1alpha1.RoleGroup{
				ObjectMeta: metav1.ObjectMeta{Name: "rg1", Namespace: "default"},
				Spec: instancev1alpha1.RoleGroupSpec{
					PostgresqlID: "test-id",
		...
		{
			name: "Sync role group - status update fails",
			roleGroup: &instancev1alpha1.RoleGroup{
				ObjectMeta: metav1.ObjectMeta{Name: "rg1", Namespace: "default"},
				Spec: instancev1alpha1.RoleGroupSpec{
					PostgresqlID: "test-id",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := reconciler.syncRoleGroup(context.Background(), tt.roleGroup, tt.info)
			...
			assert.NotNil(t, tt.roleGroup.Status.LastSyncAttempt)
			mockClient.AssertExpectations(t)
			mockStatus.AssertExpectations(t)
		})
	}
