...
// createTestGrant creates a test Grant instance
func createTestGrant(name, namespace, postgresqlID, database, role string) *instancev1alpha1.Grant {
	return &instancev1alpha1.Grant{
...
func TestGrantReconciler_Reconcile_Comprehensive(t *testing.T) {
	tests := []struct {
		name                string
	...
	}{
		{
			...
			setupMocks: func(mockClient *MockControllerClient, mockStatus *MockStatusWriter) {
				mockClient.On("Get", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
					Return(nil, errors.NewNotFound(schema.GroupResource{}, "grant"))
			},
			expectedResult: ctrl.Result{},
			expectedError:  false,
		},
		...
		},
		{
			...
			setupMocks: func(mockClient *MockControllerClient, mockStatus *MockStatusWriter) {
				grant := createTestGrant("test-grant", "default", "non-existent-id", "testdb", "testrole")
		...
		},
		{
			...
			setupMocks: func(mockClient *MockControllerClient, mockStatus *MockStatusWriter) {
				grant := createTestGrant("test-grant", "default", "test-id", "testdb", "testrole")
...
func TestGrantReconciler_Reconcile_DefaultPortAndSSLMode(t *testing.T) {
	// Arrange
	mockClient := new(MockControllerClient)
	mockStatusWriter := new(MockStatusWriter)
	logger := zap.NewNop().Sugar()
