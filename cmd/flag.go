...
func ConfigParser(cfg *config.Config) {
	...
	flag.IntVar(&cfg.WebhookServerPort, "webhook-server-port", cfg.WebhookServerPort, "Webhook server port ")
	flag.StringVar(&cfg.WebhookServerAddr, "webhook-server-addr", cfg.WebhookServerAddr, "Webhook server address")
	flag.StringVar(&cfg.WebhookK8sServiceName, "webhook-k8s-service-name", cfg.WebhookK8sServiceName, "Webhook k8s service name, using to create certificate")
	flag.StringVar(&cfg.K8sWebhookNamePostgresql, "k8s-webhook-name-postgresql", cfg.K8sWebhookNamePostgresql, "Name of webhook in k8s for postgresql kind, using to add cabundle")
	flag.StringVar(&cfg.K8sWebhookNameUser, "k8s-webhook-name-user", cfg.K8sWebhookNameUser, "Name of webhook in k8s for user kind, using to add cabundle")
	...
	flag.StringVar(&cfg.K8sNamespacePath, "k8s-namespace-path", cfg.K8sNamespacePath, "path to k8s namespace file mounted in container")
	flag.StringVar(&cfg.ExcludeUserList, "exclude-user-list", cfg.ExcludeUserList, "block creation or changing password for postgresql users list, use comma for delimit users, default 'postgres'")
